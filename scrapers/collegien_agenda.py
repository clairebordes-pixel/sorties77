...
numériques...). Sur demande de Claire, on garde TOUT (aucun filtre par
catégorie) — le champ "type" est juste une estimation grossière pour
l'affichage (concert / atelier / spectacle).
"""
import re
import sys
...
def _guess_type(categories):
    ...
    if "concert" in cats_lower or "musique" in cats_lower:
        return "concert"
    if "atelier" in cats_lower:
        return "atelier"
    return "spectacle"
...
def scrape():
    ...
    print(f"[diagnostic] nombre d'événements retenus (aucun filtre catégorie) : {len(events)}")
    return events
